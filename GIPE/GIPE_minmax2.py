...
class TreeNode:
    ...
    def __init__(self,
        ...
        self.cipher = random.uniform(encryptMin, encryptMax)
...
class OperationTree:
    ...
    def insert(self,
               ...
               label,
               encryptMin=0,
               encryptMax=C_MAX):  # encode用来记录此节点在树中的path 左0右1  
        ...
        if root == None:
            ...
            root.min = val
            root.max = val
            root.cipher = random.uniform(encryptMin, encryptMax)
...
if __name__ == "__main__":
    ...
    files = os.listdir("../ExpData/")
    for file in files:
        dataname = file
        print(dataname)
        start = time.time()
        data_path = '../ExpData/' + dataname
        ...
        Cipher = Parallel(n_jobs = min(len(X_train[0]),50), verbose = 1)(delayed(process)(col) for col in range(len(X_train[0])))

        Cipher = pd.DataFrame(Cipher)
        Cipher = pd.DataFrame(Cipher.values.T)   
        Cipher.insert(Cipher.shape[1], 'label', Y_train)   
        # print(Cipher)
        Cipher.to_csv('./OPEminmax_cipher2/' + dataname, index=0)
        X_train = pd.concat([pd.DataFrame(X_train),
                             pd.DataFrame(Y_train)],
                            axis=1)
        pd.DataFrame(X_train).to_csv('./OPEminmax_cipher2/origin_' + dataname,
                                     index=0)
        end = time.time()
